import api from '../api';
import { IContract, IContractDTO, IAppResponse, INocoDBResponse, INocoDBResponseId, IContractQueryOptions } from '@interface';

const NOCODB_TABLE_NAME = '/mohfi91mi7glsrq/records'

export enum ContractIdentifierType {
    ID = 'Id',
    UID = 'UID',
    IDENTIFIER = 'Identifier'
}

export const getNextContractIdentifier = async (): Promise<IAppResponse<string>> => {
    try {
        const { data } = await api.get<INocoDBResponse<IContract[]>>(`${NOCODB_TABLE_NAME}`, {
            params: {
                sort: '-Identifier',
                limit: 1,
            }
...
export const createContractNocodb = async (contractData: IContractDTO): Promise<IAppResponse<INocoDBResponseId>> => {
    try {
...
export const updateContractNocodb = async (
    id: string,
    input: IContractDTO
...
export const updateContractStatusNocodb = async (identifier: string, active: boolean): Promise<IAppResponse<INocoDBResponseId>> => {
    try {
        const contract = await readOneContractNocodb("IDENTIFIER", identifier);
        if (!contract) {
            return {
                code: 404,
                status: 'error',
...
export const readOneContractNocodb = async (
    type: keyof typeof ContractIdentifierType,
    identifier: string,
    options?: IContractQueryOptions
): Promise<IAppResponse<IContract>> => {
    try {
        const encodedIdentifier = encodeURIComponent(identifier);
        
        const filter = ContractIdentifierType[type];
        const params: any = {
        ...
        const { data } = await api.get<INocoDBResponse<IContract[]>>(NOCODB_TABLE_NAME, { params });
...
export const readManyContractsNocodb = async (params?: PaginationParams): Promise<IAppResponse<{
    contracts: IContract[];
    pagination: {
        currentPage: number;
        totalPages: number;
    ...
    try {
        ...
        queryString += '&nested[UserContracts][offset]=0';
        queryString += '&nested[UserContracts][limit]=25';
        ...
        const { data } = await api.get<INocoDBResponse<IContract[]>>(queryString);
...
export const deleteContractNocodb = async (id: string): Promise<IAppResponse<INocoDBResponseId>> => {
    try {
